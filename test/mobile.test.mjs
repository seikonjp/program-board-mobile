...
`;
  ...
  assert.ok(out.includes('| C-001 | 屋根の理想 | user→AI | reference | — | 屋根・graphic | グラフィック整備着手時 | new | 2026-07-15 |'), 'C-001 行が生成される');
...
`;
  ...
  assert.ok(out.includes('| C-001 | 世界のライティング | user→AI | reference | ライティング | graphics・world | S4着手時 | annotated | 2026-07-15 |'), 'C-001 行（主題=ライティング・英語ラベル）');
  assert.ok(out.includes('| C-002 | 自動調整 | user→AI | knowledge | 自動調整 | — | — | annotated | 2026-07-15 |'), 'C-002 行（種別=knowledge・主題=自動調整）');
  assert.ok(out.includes('| C-003 | 主題なし | user→AI | reference | — | — | — | new | 2026-07-15 |'), 'subject 空は主題列が —');
});
...
test('⑨ request は書き換えず consult 扱い（正規化・ラベル・往復無損失）', () => {
  ...
  assert.ok(out.includes('| C-004 | 玄関配置の相談 | user→AI | consult | 玄関 | — | — | new | 2026-07-15 |'), 'request 行は種別=consult で出力');
});
...
test('⑬ decision 型: ラベル・抽出・往復無損失・buildNewCardMarkdown（v1.5）', () => {
  ...
  assert.ok(idx.includes('| C-0003 | Q-04 | AI→user | decision | ライティング | — | — | new | 2026-07-15 |'), 'decision 行が生成される');
