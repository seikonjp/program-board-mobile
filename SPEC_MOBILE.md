...
> **build29（v2.9・Progressビュー進捗軸対応・2026-07-17・SPEC_V2 §3-2改定）**: Progressを**作業単位（work unit）一望**へ拡張。全源結合＝**feature/fpu**（FEATURE_FPU_CENSUS）＋**com**（COM_FUNCTION_TARGETS・新アダプタ）＋**data/infra/framework/doc/ui/test**（`Program/PROGRESS_AXIS.md` §6の新パーサ`parseProgressAxis`）。**統一状態語彙6値**（未着手/待ち/進行中/完了/対象外/不明・§2吸収表）で色分け・**待ちはblocked_reasonチップ**。stage×kind×statusのグルーピング切替＋フィルタ＋検索。**フロンティア表示**＝depsを辿り未完の依存を明示・**逆引き**（Q-*/S*/単位を選ぶと推移的に依存する全単位を点灯＝`reverseClosure`・循環安全）。重ね合わせ＝TASK_LEDGER/LANES/test_status＋**CARRYOVER申し送りバッジ**。純関数（`parseProgressAxis`/`parseComTargets`/`deriveComStatus`/`normalizeWorkStatus`/`parseCarryover`/`buildWorkUnits`/`reverseClosure`/`buildProgressPayload`）は`parser.js`・Mac版`server.js`と挙動互換・**PROGRESS_AXIS未存在/空表でも壊れない**。ソースは`config.js` progressSources（comTargets/progressAxis/carryover追加）。status`awaiting-impl`（実装待ち・保留グループ→マーカー）を`STATUS_ORDER`/`STATUS_LABEL`/`treatmentMarker`へ追加。
>
> **build27（v2.8・status表示語彙整理＋処遇マーカー＋完了ビュー・2026-07-17）**: 内部status値（英語）は不変・**表示のみ平易語**（`annotated→確認済み`／`review→対応待ち`／`responded→AI対応中`／`consumed→完了`／`waiting→保留`／`carried→申し送り`〔新設・`STATUS_ORDER`登録〕／reference・knowledge既定=`参考`・具体的statusは優先）。**処遇マーカー**（`card-tile-title`末尾右・statusから自動導出＝別フィールドにしない）: ✓塗り=完了／✓中空=完了提案／→=保留グループ／なし=対応中。純関数`statusLabel(status,type)`/`treatmentMarker(status,type)`（`parser.js`・Mac版と同一ロジック・テスト付き）。**完了ビュー**（新タブ「完了」＝`views/completed.js`・`completedCards`＝consumed＋アーカイブを一望・Board/type別タブは`listableCards`でconsumed除外・検索到達可）。carriedも完了ボタン対象（→consumed・`confirmDone`のデータ層検証も同期）。
>
